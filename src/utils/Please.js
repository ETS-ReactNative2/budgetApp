...
;(function(globalName, root, factory) {
    ...
    function define_Please() {
        ...
        const make_scheme_default = {
            scheme_type: 'analogous',
            format: 'hex',
        }

        const make_contrast_default = {
            golden: false,
            format: 'hex',
        }
        ...
        function convert_to_format(format_string, array) {
            ...
            switch (format_string) {
                ...
                case 'rgb-string':
                    for (i = 0; i < array.length; i++) {
                        const raw_rgb = Please.HSV_to_RGB(array[i])
        ...
        function contrast_quotient(HSV) {
            const RGB = Please.HSV_to_RGB(HSV)
        ...
        function RC4Random(seed) {
            const key_schedule = []
            let key_schedule_i = 0
            let key_schedule_j = 0
            for (let k = 0; k < 256; k++) key_schedule[k] = k
            for (let i = 0, j = 0; i < 256; i++) {
                j = (j + key_schedule[i] + seed.charCodeAt(i % seed.length)) % 256
                const t = key_schedule[i]
            ...
            function get_random_byte() {
                ...
                const t = key_schedule[key_schedule_i]
        ...
        Please.RGB_to_HSV = function(RGB) {
            ...
            let computed_V = 0
            const min_RGB = Math.min(r, Math.min(g, b))

            const max_RGB = Math.max(r, Math.max(g, b))
            // Black-gray-white
            if (min_RGB === max_RGB) {
                computed_V = min_RGB
                return {
            ...
            const d = r === min_RGB ? g - b : b === min_RGB ? r - g : b - r
            const h = r === min_RGB ? 3 : b === min_RGB ? 1 : 5
        ...
        Please.make_scheme = function(HSV, options) {
            ...
            const scheme_options = copy_object(make_scheme_default)
        ...
        Please.make_color = function(options) {
            ...
            const color_options = copy_object(make_color_default)
            ...
            for (let i = 0; i < color_options.colors_returned; i++) {
                const random_hue = random_int(0, 360, randomiser)
        ...
        Please.make_contrast = function(HSV, options) {
            // clone base please options
            const contrast_options = copy_object(make_contrast_default)
            ...
            const value_range = contrast_quotient(HSV)
            ...
            if (contrast_options.golden === true) {
                ...
                const contrast_base = Please.make_scheme(HSV, {
